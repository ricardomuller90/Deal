...
<script>
    ...
    function calculo(){
        ...
        var ints = (subtotal *30/100) * <?php echo $reflexos;?>;
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-8">
                        <div class="card">
                            <div class="header">
                                <h4 class="title">Cálculo de Horas Extras SEM Aumento da média remuneratória <a href="#modal"><img src="../img/ajuda.png" width="30"></a></h4>
                            </div>
                            <div class="content">
...
<?php   $PDO = db_connect();      
        $stmt = $PDO->prepare('SELECT horas_exAum.valor, horas_exAum.numHoras, horas_exAum.adicional, horas_exAum.numHorasPleit, horas_exAum.mesesTrabalhados, processo.remHab as remHab, difSalariais.valor as difSal,insalubridade.adicional as insAdic ,insalubridade.salarioMin as insSalMin , periculosidade.salarioMin as perSalMin FROM horas_exAum INNER JOIN processo ON horas_exAum.idProcesso=processo.idProcesso INNER JOIN insalubridade ON processo.idProcesso=insalubridade.idProcesso INNER JOIN periculosidade ON periculosidade.idProcesso=horas_exAum.idProcesso INNER JOIN difSalariais ON difSalariais.idProcesso=processo.idProcesso  WHERE processo.idProcesso = :idProcesso ');
        $stmt->bindValue(':idProcesso', $_GET['idProcesso']);
        $stmt->execute();
        $valores = $stmt->fetchAll(PDO::FETCH_ASSOC);
        foreach ($valores as $row){
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="valor" name="valor" class="form-control" readonly="readonly" value="<?php if(isset($valor)){echo $valor;}?>">
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                  <label>Carga horária mensal</label>
                                                <input type="text" id="numHoras" name="numHoras" class="form-control" maxlength="3" placeholder="220" value="<?php if(isset($row['numHoras'])){echo $row['numHoras'];}?>">
                                            </div>
                                        </div>
                                    </div>
 ...
 <div class="remodal" data-remodal-id="modal">
  ...
  <h1>Cálculo de Horas Extras SEM Aumento da média remuneratória</h1>
  <p>
  ...
  <p style="font-weight: bolder"> Remuneração habitual mensal paga + pleitos mensais / carga horária mensal x adic. Horas extras x número de horas extras x número de meses obedecendo a prescrição + 30% (referente as integrações de aviso, 13º. Salários e férias c/1/3), caso seja escolhido esta opção na página inicial dos cálculos.
    <br><br></p>
...
<div class="remodal" data-remodal-id="modal3">
  <button data-remodal-action="close" class="remodal-close"></button>
  <h1> Digitar o número de horas extras pleiteadas neste mês.</h1>
  <p>
   Dica para encontrar o número de horas extras por mês:</br>
Somar a carga horária laborada por dia e multiplicar pelo número de dias laborados e subtrair pela carga semanal contratada (exemplo: 36, 40,44), este resultado será o número de horas extras semanais, sendo assim, para terem o resultado mensal bastam multiplicar este número de horas extras semanais x 4,29 semanas (média real aritmética do número de semanas = 30 dias : 7 dias.</br>
Exemplo – carga horária 44hs.:</br>
10 hs. laboradas por dia x 6 dias = 60hs. – 44hs. = 16hs. extras semanais x 4,29 semanas = 68,64hs. extras por mês   
  </p>
