...
function criarConta($username, $pass, $interv) {
	global $PDO;
	$k = $interv;
...
function extenderConta($username, $termino_previsto, $interv) {
	$k = $interv;
...
function getInfoInterval($info) {
    if(!is_object($info)) {
        basicLog("Invalid info:".print_r($info, true));
    }
    if( (!isset($info->receipt)) || (!is_object($info->receipt)) ) {
        basicLog("Receipt was not found found on info object. Returning 30 as default interval:".print_r($info, true));
        return 30;
    }

    if( (!isset($info->receipt->in_app)) || (!is_array($info->receipt->in_app)) ) {
        basicLog("Receipt transactions were not found found on info object. Returning 30 as default interval:".print_r($info, true));
        return 30;
    }

    $ct = count($info->receipt->in_app);
    if($ct === 0) {
        basicLog("Empty transactions for info: ".print_r($info, true));
        return 30;
    }

    if( (!is_object($info->receipt->in_app[$ct-1])) || (!isset($info->receipt->in_app[$ct-1]->product_id)) ) {
        basicLog("Couldnt find last transaction for info: ".print_r($info, true));
        return 30;
    }

    $pid = $info->receipt->in_app[$ct-1]->product_id;
    switch($pid) {
        case 'assinaturamensal':
            return 30;
        case 'assinaturatrimestral':
            return 90;
        case 'assinaturasemestral':
            return 180;
    }
    return 30;//default
}
...
function processPost() {
    ...
    basicLog("RECEIPT:". print_r($post['appStoreReceipt'], true));
    basicLog("USER:". print_r($post['username'], true));
    ...
    $interv = getInfoInterval($info);
    basicLog("Interval found:".print_r($interv, true));
    try{
    ...
    if(empty($r) || count($r) == 0) {
    	if(!criarConta($post['username'], $post['pass'], $interv)) {
    ...
    if(!extenderConta($post['username'], $r[0]['final_assinatura'], $interv)) {//atualiza o final_assinatura
